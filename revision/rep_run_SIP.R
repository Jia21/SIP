...
if(as.numeric(num) <= 90){
cell = "GM12878"
rep = as.numeric(num) - 88
}else if (as.numeric(num) > 90){
cell = "K562"
rep = as.numeric(num) - 90
}
...
k <- fread(paste0(mapsDir, "SRR58314",num,".10k.2.peaks.bedpe"))
...
fwrite(sips, paste0(cell,"_rep_",rep,"_SIPs.txt"), sep="\t", col.names=T)
...
ggplot(score, aes(x=rank, y=intscore)) + 
  ...
  ggtitle(paste0(cell, " H3K27ac HiChIP SIPs (replicate ", rep,")"))+
  theme_bw()
ggsave(paste0("./figures/",cell,"_rep_",rep,"_SIPs.png"), width=6, height=5)
...
ggsave(paste0("./figures/",cell,"_rep_",rep,"_score.png"), width=5, height=6)
...
ggplot(data=nPIR, aes(x=SIP, y=log10(N), fill=as.factor(SIP))) + 
  ...
  xlab("Bait Type") + 
  theme_bw()+theme(legend.title = element_blank())
ggsave(paste0("./figures/",cell,"_rep_",rep,"_PIR.png"), width=5, height=6)
#----------------------------------------------------------------------------------------------#
#----------------------------------------------------------------------------------------------#
