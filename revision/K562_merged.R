...
fwrite(sips, "K562_combined_r1r2r3.10k.2.peaks_SIPs.txt", sep="\t", col.names=T)
...
ggplot(score, aes(x=rank, y=intscore)) + 
  ...
  ggtitle("K562 H3K27ac HiChIP SIPs (merged)")+
  theme_bw()
ggsave("./figures/K562_combined_r1r2r3.10k.2.peaks_SIPs.png", width=6, height=5)
...
ggsave("./figures/K562_combined_r1r2r3.10k.2.peaks_score.png", width=5, height=6)
...
ggplot(data=nPIR, aes(x=SIP, y=log10(N), fill=as.factor(SIP))) + 
  ...
  xlab("Bait Type") + 
  theme_bw()+theme(legend.title = element_blank())
ggsave("./figures/K562_combined_r1r2r3.10k.2.peaks_PIR.png", width=5, height=6)
#----------------------------------------------------------------------------------------------#
#----------------------------------------------------------------------------------------------#
