...
setwd("/proj/yunligrp/users/jwen/SIP/revise/3_5")
# setwd("/proj/yunligrp/users/lagler/SIP/data/")

shared <- fread("/proj/yunligrp/users/lagler/SIP/data/Shared_genes_allInfo.txt", drop=c(2,4:6))
...
gtex <- fread("/proj/yunligrp/users/jwen/SIP/post/GTEx_mat_tissue_median", drop=12)
...
t <- exprdata3[exprdata3$TopPerBlood.x == 10,]
table(t$color)
summary(t$medTPM[t$color =="Blood Cell"])
summary(t$medTPM[t$color =="Other Tissue"])
summary(t$medTPM[t$color =="Whole Blood"])