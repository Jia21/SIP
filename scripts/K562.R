...
mapsDir <- "/Users/tmlagler/OneDrive/Lab/SIP/figurecode/data/"
k <- fread(paste0(mapsDir, "K562_combined_r1r2r3.10k.2.peaks.bedpe"))

# hg19 promoters from gencode
# gencodeDir <- "/proj/yunligrp/users/ycyang/Gencode/"
gencodeDir <- "/Users/tmlagler/OneDrive/Lab/treearvin/inputs/"
gencode <- fread(paste0(gencodeDir, "gene_info.gencode.v28lift37.txt"),
...
fwrite(sips, "K562_SIPs.txt", sep="\t", col.names=T)
...
ggplot(score, aes(x=rank, y=intscore)) + 
  ...
  ggtitle("K562 HiChIP SIPs")+
  theme_bw()
ggsave("../figures/K562_SIPs.png", width=6, height=5)
...
ggsave("../figures/K562_score.png", width=5, height=6)
...
ggplot(data=nPIR, aes(x=SIP, y=log10(N), fill=as.factor(SIP))) + 
  ...
  xlab("Bait Type") + 
  theme_bw()+theme(legend.title = element_blank())
ggsave("../figures/K562_PIR.png", width=5, height=6)
#----------------------------------------------------------------------------------------------#
#----------------------------------------------------------------------------------------------#
